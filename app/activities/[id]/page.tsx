...
import {
  ArrowLeft, Clock, Users, IndianRupee,
  CheckCircle, Loader2, CalendarDays, Star, ShieldCheck, UserPlus
} from "lucide-react"
import { Button } from "@/components/ui/button"
import { Badge } from "@/components/ui/badge"
import { api, ActivityDetail } from "@/lib/api"
    ...
    <main className="min-h-screen bg-background">
      ...
      <div className="relative w-full h-[50vh] md:h-[65vh]">
        ...
        <div className="absolute top-6 right-6 flex flex-col gap-2 items-end">
          {activity.is_popular && (
            <Badge className="bg-accent text-accent-foreground px-3 py-1">
              <Star size={12} className="mr-1" fill="currentColor" /> Most Popular
            </Badge>
          )}
          {activity.requires_prebooking && (
            <Badge className="bg-blue-600 text-white px-3 py-1">Pre-book Only</Badge>
          )}
        </div>
        ...
        <div className="md:col-span-2 flex flex-col gap-10">
          ...
          {activity.rules && activity.rules.length > 0 && (
            <section>
              <div className="flex items-center gap-2 mb-4">
                <ShieldCheck size={18} className="text-accent shrink-0" />
                <h2 className="text-xl font-bold text-white">Rules & Safety Guidelines</h2>
              </div>
              <ul className="flex flex-col gap-3">
                {activity.rules.map((item) => (
                  <li
        ...
        <div className="md:col-span-1">
          <div className="sticky top-6 bg-card border border-border rounded-2xl p-6 flex flex-col gap-5 shadow-xl">
            ...
            <div>
              ...
              {/* per person / per group label */}
              <p className="text-xs text-muted-foreground mt-1">
                {isPerPerson
                  ? `per adult • min ${activity.min_persons} person${activity.min_persons !== 1 ? "s" : ""}`
                  : `per group • up to ${activity.min_persons} persons`}
              </p>
              ...
              {/* ✅ Extra person charge callout */}
              {hasExtra && (
                <div className="mt-3 flex items-start gap-2 bg-yellow-500/10 border border-yellow-500/30 rounded-xl px-3 py-2.5">
                  <UserPlus size={14} className="text-yellow-400 mt-0.5 shrink-0" />
                  <div>
                    <p className="text-yellow-400 text-xs font-semibold">
                      +₹{extraCharge.toLocaleString()} per extra person
                    </p>
                    <p className="text-yellow-400/70 text-[11px] mt-0.5">
                      Applied for each person beyond {activity.min_persons}
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="flex flex-col gap-3 text-sm">
              <div className="flex items-center gap-3 text-muted-foreground">
                ...
                <span>
                  Duration:{" "}
                  <span className="text-white font-medium">{activity.duration}</span>
                </span>
              </div>
