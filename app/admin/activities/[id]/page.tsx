...
import {
  ...
  BookOpen, LogOut, Palmtree, Plus,
  Trash2, Upload, Save, Clock, Pencil,
  Menu, X, Settings, ShieldCheck, Baby,
} from "lucide-react"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
...
export default function ActivityFormPage() {
  ...
  const [form, setForm] = useState({
    ...
    base_price:          "",
    child_price:         "",
    min_age:             "",   // ← NEW: blank = all ages allowed; set e.g. "7" = no children under 7
    pricing_type:        "per_person",
    extra_person_charge: "",
    min_persons:         "1",
    max_persons:         "10",
    ...
    Promise.all([
      ...
      .then(([activity, rulesData]: any) => {
        setForm({
          ...
          base_price:          String(activity.base_price   ?? ""),
          child_price:         activity.child_price != null  ? String(activity.child_price)  : "",
          min_age:             activity.min_age     != null  ? String(activity.min_age)      : "",  // ← NEW
          pricing_type:        activity.pricing_type        ?? "per_person",
          extra_person_charge: String(activity.extra_person_charge ?? ""),
          min_persons:         String(activity.min_persons  ?? "1"),
          max_persons:         String(activity.max_persons  ?? "10"),
  ...
  const handleSave = async () => {
    ...
    if (form.min_age !== "") {
      const age = parseInt(form.min_age, 10)
      if (isNaN(age) || age < 1 || age > 99) {
        toast.error("Minimum age must be between 1 and 99")
        return
      }
    }
    ...
    try {
      const payload = {
        ...
        base_price:          parseFloat(form.base_price)          || 0,
        child_price:         form.child_price ? parseFloat(form.child_price)   : null,
        min_age:             form.min_age     ? parseInt(form.min_age,   10)   : null,  // ← NEW
        extra_person_charge: parseFloat(form.extra_person_charge) || 0,
        min_persons:         parseInt(form.min_persons,   10)     || 1,
        max_persons:         parseInt(form.max_persons,   10)     || 10,
        display_order:       parseInt(form.display_order, 10)     || 0,
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 lg:gap-6">
          ...
          <div className="md:col-span-2 flex flex-col gap-5">
            ...
            <section className="bg-card border border-border rounded-2xl p-5 flex flex-col gap-4">
              ...
              {/* ── Minimum Age ── NEW ────────────────────────────── */}
              <div className="rounded-xl border border-border/60 bg-background/50 p-4 flex flex-col gap-3">
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-lg bg-orange-500/15 flex items-center justify-center shrink-0">
                    <Baby size={13} className="text-orange-400" />
                  </div>
                  <p className="text-white text-sm font-semibold">Child Age Restriction</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 items-end">
                  ...
                  <div className="pb-0.5">
                    {form.min_age === "" ? (
                      <div className="flex items-center gap-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-3 py-2.5 h-11">
                        <span className="text-emerald-400 text-xs font-medium">✓ All ages allowed</span>
                      </div>
                    ) : (
                      <div className="flex items-center gap-1.5 bg-orange-500/10 border border-orange-500/20 rounded-lg px-3 py-2.5 h-11">
                        <Baby size={12} className="text-orange-400 shrink-0" />
                        <span className="text-orange-400 text-xs font-medium">
                          Under {form.min_age} yrs not allowed
                        </span>
                      </div>
                    )}
                  </div>
                ...
                <p className="text-xs text-muted-foreground">
                  {form.min_age === ""
                    ? "Leave blank to allow all ages including children."
                    : `Children under ${form.min_age} years will not be allowed. The children counter will be hidden on the booking form.`}
                </p>
              </div>
              {/* ── End Minimum Age ───────────────────────────────── */}
              ...
              {form.child_price === "" && form.min_age === "" && (
                <p className="text-xs text-muted-foreground bg-accent/5 border border-accent/15 rounded-lg px-3 py-2">
                  💡 Child Price is blank — children will be charged the same as adults.
                </p>
              )}
              {form.child_price !== "" && form.min_age !== "" && (
                <p className="text-xs text-muted-foreground bg-orange-500/5 border border-orange-500/15 rounded-lg px-3 py-2">
                  ⚠️ Child Price is set but children under {form.min_age} years are not allowed — the child price will only apply to children aged {form.min_age}+.
                </p>
              )}
            </section>
